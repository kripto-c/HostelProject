...
export default function ClientEdit() {
  ...
  const upLoadImage = async (e) => {
    ...
    setLoading(true);
    const res = axios("http://api.cloudinary.com/v1_1/drw5h95um/image/upload", {
      method: "POST",
      body: data,
    });
      ...
      >
        <div>
          ...
          <input
            type="file"
            name="file"
            placeholder="Insertar imagen aqui..."
            onChange={upLoadImage}
          >{image? image : 'Image not Found'}</input>
        </div>
        ...
        <div className="col-md-12">
          ...
          <div className="input-group">
            <input
              ...
              disabled={provin}
              onChange={(e) => handleChange(e)}
              value={provin ? info.nationality : client.nationality}
              required
            />
            <button
              key={"btnNamesc"}
              className="btn btn-outline-danger"
              type="button"
              onClick={(e) => handleProvin(e)}
            >
              <BsFillPencilFill />
            </button>
          </div>
          <div className="invalid-feedback">Por favor indique su Provincia</div>
        </div>
        <div className="col-md-6">
